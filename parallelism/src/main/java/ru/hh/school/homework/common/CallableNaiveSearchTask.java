...
public class CallableNaiveSearchTask implements Callable {

    private String query;

    private Path directoryPath;

    public CallableNaiveSearchTask(String query, Path directoryPath) {
        this.query = query;
    ...
    public Long call() {
        try {
            return printAndGetGoogleCount();
        } catch (IOException e) {
            return -1L;
        }
    }

    private Long printAndGetGoogleCount() throws IOException {
        Long count = naiveSearch(query); //naiveSearchMock()
        System.out.printf("%s - %s - %d\r\n", directoryPath, query, count);
        return count;
    }
